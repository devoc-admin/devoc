...
export function Safari({
  ...
  useEffect(() => {
    if (!video || invisibleMode || !isActive) return;
  ...
  }, [video, videoDelay, invisibleMode, isActive]);
  ...
  // Reset video state when isActive changes (e.g., achievement navigation)
  useEffect(() => {
    if (!isActive) {
      setShowVideo(false);
      if (videoRef.current) {
    ...
    >
      {invisibleMode ? (
        ...
        <div className={cn(
          ...
          "border-primary border-t border-r",
          "bg-slate-900/30",
          "backdrop-sm"
        )} />
      ) : (
          ...
          <Image
            alt="Réalisation"
            className="absolute block size-full object-cover object-top"
            height={1200}
            src={image}
            width={1200}
            style={mediaStyle}
            ...
            <>
              ...
              {showVideo && (
                <VideoPlayButton
                  ...
                  style={{
                    right: `${100 - LEFT_PCT - WIDTH_PCT + 1.5}%`,
                    bottom: `${100 - TOP_PCT - HEIGHT_PCT + 2}%`,
                  }}
                />
              )}
            </>
          )}
            <BrowserLayout image={image} url={url} />
        </>
      )}
    </div>
  );
...
// --------------------------------
// 🌐 Browser layout
function BrowserLayout({ image, url }: { image: StaticImageData; url: string | undefined}) {
return  <svg
...
>
  ...
  <g
    clipPath="url(#path0)"
    mask={image ? "url(#safariPunch)" : undefined}
  >
    <path
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "rounded-full bg-zinc-800/70 p-0",
