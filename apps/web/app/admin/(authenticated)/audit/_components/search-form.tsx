                ...
                {(field) => (
                  <div>
                    <Label className="font-kanit text-lg">
                      Nombre maximum de pages récupérées
                    </Label>
...
function useSearchForm() {
  const { handleCrawlJobId } = useAuditContext();
  const form = useForm({
    defaultValues: {
    ...
    onSubmit: async (values) => {
      ...
      const result = await upsertAudit({ maxDepth, maxPages, url: search });

      if (result.success) {
        toast("Le site a bien été ajouté ! L'audit est en cours...", {
          icon: "✅",
