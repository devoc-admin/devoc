...
When an element has many Tailwind classes, split them into multiple strings using a `cn()` utility. Group classes logically by category:
...
`cn()` arrives with `bun x shadcn@latest init` (see the readme); until then keep class lists short.
